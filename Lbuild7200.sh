...
setup_build_mode() {
	...
	if [ "$RELEASE" == "rel" ]; then
		lunch msm8952_64-user 
	else
		lunch msm8952_64-userdebug 
	fi
	
	./prebuilts/sdk/tools/jack-admin kill-server
	./prebuilts/sdk/tools/jack-admin start-server
	export TARGET_USES_GMS_PACKAGES=true
}
#1
build_kernel() {
...
build_android() {
	...
	if [ -e $PRODUCT_OUT/system.img ]; then
	    ...
	    cp -u $PRODUCT_OUT/mdtp.img $BIN_PATH/	
	else
...
flash_android() {
	...
	if [ -e $BIN_PATH/mdtp.img ]; then
	    echo "> fastboot flash mdtp mdtp.img"
	    fastboot flash mdtp $BIN_PATH/mdtp.img	
	else
	    echo "XXXXXX [ error: cannot load 'mdtp.img' !! ] XXXXXX"
	fi
	fastboot reboot		
}
